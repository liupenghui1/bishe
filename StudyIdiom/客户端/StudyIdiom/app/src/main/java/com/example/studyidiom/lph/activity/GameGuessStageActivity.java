...
public class GameGuessStageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.lph_activity_game_guess_stage);
        gameStageView = findViewById(R.id.gsv);
        Intent intent=getIntent();
        nowPosition = Integer.parseInt(intent.getStringExtra("guess_nowPosition"));
        Log.e("nowPositionStage",nowPosition+"");
        if(nowPosition>0){
            ...
            gameStageView.setOnGameItemClickListener(new GameStageView.OnGameItemClickListener() {
                ...
                public void onGameItemClick(int position, boolean isLock) {
                    if (position > nowPosition + 1) {
                        ...
                        Intent intent = new Intent(GameGuessStageActivity.this, PlayGuessActivity.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode==100 && resultCode==200){
            ...
            plusGameGuessSet(uid,nowPosition);
            Log.e("hello",nowPosition+"");
        }
    }
    private void plusGameGuessSet(String uId,int nowset) {
        new Thread() {
            @Override
            public void run() {
                try {
                    String string = getResources().getString(R.string.lphipConfig);
                    URL url = new URL("http://" + string + ":8080/InternetServer/lph/updateGuessUser/" + uId+"/"+nowset);
